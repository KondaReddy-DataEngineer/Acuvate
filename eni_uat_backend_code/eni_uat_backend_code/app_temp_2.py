...
def extract_label_with_llm(cropped_roi_path: str) -> str:
    ...
    2. Normalize the tag using strict rules:
       ...
       - If a tag contains a combined suffix in the form A/B at the end (e.g., 1158A/B), split it into two normalized tags, inserting a hyphen before each suffix: e.g., 1158A/B → 1158-A and 1158-B.
       - If a tag contains a combined suffix in the form C/D followed by additional sections (e.g., 1158C/D-1), split it into two normalized tags, inserting a hyphen before each suffix while preserving the trailing section: e.g., 1158C/D-1 → 1158-C-1 and 1158-D-1.
       - Except for the suffix-splitting rules above, do NOT create multiple tags. Extract only the single visible tag.
