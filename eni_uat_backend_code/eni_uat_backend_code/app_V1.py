...
def extract_component_labels_1(components, cropped_image_path, filename):
    img = cv2.imread(cropped_image_path)
    if img is None:
        log("ERROR: Could not load image for OCR.")
        return components
    ...
    # Add center to every component
    for comp in components:
        x1,y1,x2,y2 = comp["bbox"]
        comp["center"] = [ (x1+x2)//2, (y1+y2)//2 ]

    # Separate detection groups
    mixed_names = ['Compressor or Vessel','Tank','Tank 2']
    outside_only_names = [
        'Blower','Cooler 2','Cooler 3','Filter','Pump','Positive Displacement Meter'
    ]

    tag_items = [c for c in components if c["name"] == "Tag"]
    target_components = [c for c in components if c["name"] in mixed_names + outside_only_names]
    ...
    for comp in target_components:
        ...
        # Only mixed_names are allowed to have internal text
        if comp["name"] in mixed_names:
            roi = img[y1:y2, x1:x2]
            crop_path = os.path.join(crop_dir, f"inside_{comp['id']}.png")
            cv2.imwrite(crop_path, roi)
            inside_label = extract_label_with_llm(crop_path)
        ...
        if nearest and best_dist < DIST_THRESHOLD:
            ...
            cv2.imwrite(crop_path, roi)
            outside_label = extract_label_with_llm(crop_path)

        # FINAL LABEL RULE
        if comp["name"] in mixed_names:
            # priority: inside → outside
            comp["label"] = inside_label or outside_label or ""
        else:
            # outside_only_names
            comp["label"] = outside_label or ""

    # Normalize + id + store associated class name
    for comp in components:
        x1,y1,x2,y2 = comp["bbox"]
        comp["topX"] = x1/W
        comp["topY"] = y1/H
    ...
    log("✔ Finished updated inside/outside Tag labeling (simplified)")
    return components
