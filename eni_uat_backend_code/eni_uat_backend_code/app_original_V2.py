...
########saikumar code##################################
#################################longline#################################################################
# def detect_and_connect_lines(image_path, output_path=None):
#     """
#     Detect line segments and connect them, handling perpendicular segments.
...
#             if angle < 30:  # Horizontal (0-30 degrees)
#                 horizontal_lines.append((x1, y1, x2, y2))
#             elif angle > 60:  # Vertical (60-90 degrees)
#                 vertical_lines.append((x1, y1, x2, y2))
        
#         # Connect horizontal segments
#         if horizontal_lines:
#             horizontal_lines = connect_collinear_segments(horizontal_lines, orientation='horizontal')
        
#         # Connect vertical segments
#         if vertical_lines:
#             vertical_lines = connect_collinear_segments(vertical_lines, orientation='vertical')
...
#         # Connect perpendicular intersections
#         result = connect_perpendicular_lines(result, horizontal_lines, vertical_lines)
   
#     if output_path:
#        # cv2.imwrite(OUTPUT_DIR/+"output_path", result)
#         #full_output_path =cv2.imwrite(os.path.join(OUTPUT_DIR, output_path), result)
#         os.makedirs(OUTPUT_DIR, exist_ok=True)
...
# def connect_collinear_segments(segments, orientation, threshold=5, small_gap=30, large_gap=200):
#     """
#     Connect collinear segments:
#     - Small gaps: always connect.
#     - Large gaps: connect only if path is clear (no other segment blocking).
#     """
#     if not segments:
#         return []
...
#     segments = sorted(segments, key=lambda s: (s[1], s[0]) if orientation=='horizontal' else (s[0], s[1]))
...
#                     gap = min(abs(x1 - x3), abs(x1 - x4), abs(x2 - x3), abs(x2 - x4))

#                     if gap < small_gap:
#                         # Always connect small gaps
#                         x1 = min(x1, x2, x3, x4)
#                         x2 = max(x1, x2, x3, x4)
#                         y1 = y2 = int((y1 + y3) / 2)
#                         used[j] = True
...
#                     elif gap < large_gap:
#                         # Check if "path" is clear (no vertical lines between)
#                         mid_x = (max(x1, x2, x3, x4) + min(x1, x2, x3, x4)) // 2
#                         if not any(abs(mid_x - sx) < threshold for (sx, sy1, sx2, sy2) in segments if abs(sx2-sx) < 5):
#                             x1 = min(x1, x2, x3, x4)
#                             x2 = max(x1, x2, x3, x4)
#                             y1 = y2 = int((y1 + y3) / 2)
#                             used[j] = True
...
#                     gap = min(abs(y1 - y3), abs(y1 - y4), abs(y2 - y3), abs(y2 - y4))
...
#                     elif gap < large_gap:
#                         mid_y = (max(y1, y2, y3, y4) + min(y1, y2, y3, y4)) // 2
#                         if not any(abs(mid_y - sy) < threshold for (sx1, sy, sx2, sy2) in segments if abs(sy2-sy) < 5):
#                             y1 = min(y1, y2, y3, y4)
#                             y2 = max(y1, y2, y3, y4)
#                             x1 = x2 = int((x1 + x3) / 2)
#                             used[j] = True
...
# def connect_perpendicular_lines(img, h_lines, v_lines, threshold=15):
#     """
#     Extend and connect perpendicular intersections instead of drawing circles.
    
#     Args:
#         img: Image to draw on
#         h_lines: List of horizontal lines
#         v_lines: List of vertical lines
#         threshold: Max distance to consider a connection
#     """
#     result = img.copy()

#     for hx1, hy1, hx2, hy2 in h_lines:
#         hy = int((hy1 + hy2) / 2)  # average y

#         for vx1, vy1, vx2, vy2 in v_lines:
#             vx = int((vx1 + vx2) / 2)  # average x

#             # Check if they are close enough to connect
#             if (min(hx1, hx2) - threshold <= vx <= max(hx1, hx2) + threshold and
#                 min(vy1, vy2) - threshold <= hy <= max(vy1, vy2) + threshold):

#                 # Extend horizontal line to meet vertical
#                 if not (hx1 <= vx <= hx2):
#                     hx1 = min(hx1, vx)
#                     hx2 = max(hx2, vx)

#                 # Extend vertical line to meet horizontal
#                 if not (vy1 <= hy <= vy2):
#                     vy1 = min(vy1, hy)
#                     vy2 = max(vy2, hy)

#                 # Draw extended lines in green
#                 cv2.line(result, (hx1, hy), (hx2, hy), (0, 255, 0), 2)
#                 cv2.line(result, (vx, vy1), (vx, vy2), (0, 255, 0), 2)
...
def yolo_model_line_detection(input_image_path, output_dir, model_path="best.pt"):
    ...
    # print(f"Saved line detection JSON: {lines_json_path}")
    # Save line detection JSON
    ######################################
    #lines_json_path = os.path.join(output_dir, f'{filename}_Lines_detect.json')

    # Connect overlapping line segments
...
# ============================================================================
# def preprocess_roi_adaptive(roi):
#     """Apply adaptive thresholding preprocessing to enhance text for OCR"""
#     if len(roi.shape) == 3:
...
#     """Upscale image for better OCR results on small text"""
...
# def Component_Text_Extraction(model_name: str, full_image_path: str, filename: str):
#     """
#     1. Load detections JSON.
#     2. For every ROI that belongs to a label of interest:
#          • Crop the ROI → save temporarily.
#          • Send the crop to GPT-4o → get normalised tag.
#          • Store the tag back into the JSON.
#     3. Continue with your existing OCR + regex pipeline (optional).
#     """
#     out_dir = "images"
#     json_file = os.path.join(out_dir, f"{filename}_Detections.json")
...
#     if img is None:
#         logger.error(f"Cannot read {full_image_path}")
#         return
#     H, W = img.shape[:2]

#     # pixel bbox
#     def norm2px(box, margin=5):
#         return (
#             max(0, int(box["topX"]    * W) - margin),
#             max(0, int(box["topY"]    * H) - margin),
#             min(W, int(box["bottomX"] * W) + margin),
#             min(H, int(box["bottomY"] * H) + margin)
#         )
...
#         "Installed in Control Panel","Tag","Blower","Tank",
#         "Compressor or Vessel","Cabinet or Chamber","Pump"
#     }

#     # Temporary folder for crops
#     crop_dir = os.path.join(out_dir, "label_crops")
#     os.makedirs(crop_dir, exist_ok=True)

#     # Process each ROI
#     for idx, item in enumerate(data["text_data"]):
#         if item.get("label") not in labels_of_interest:
#             continue

#         left, top, right, bottom = norm2px(item, margin=10)
#         roi = img[top:bottom, left:right]

#         # ---- Save crop (optional – useful for debugging)
#         crop_path = os.path.join(crop_dir, f"{filename}_roi_{idx}.png")
#         cv2.imwrite(crop_path, roi)

#         # ---- Ask LLM for the tag
#         llm_tag = extract_label_with_llm(crop_path)
#         if llm_tag:
#             data["text_data"][idx]["label"] = llm_tag
#             logger.info(f"ROI {idx} → LLM tag: {llm_tag}")


#     # Save final JSON
#     final_path = os.path.join(out_dir, f"{filename}_Json_results_OCR.json")
...
# def process_json(json_file_path: str) -> Dict:
#     """
#     Process JSON to check labels in specific text_associated values and assign from closest tags
#     if label is empty, 1-2 digits, or 1-2 letters.
#     Also checks if label and text_associated are the same value.
#     Updates the input JSON file directly and creates a detailed version.
    
#     Args:
#         json_file_path: Path to the input JSON file
    
#     Returns:
#         Detailed JSON data with assigned_from_tag_id field
#     """
#     # text_associated values to check
#     target_text_associated = [
#         "Blower",
#         "Tank",
#         "Compressor or Vessel",
#         "Cabinet or Chamber",
...
#     # Create deep copy for detailed version
#     import copy
#     detailed_json = copy.deepcopy(json_data)
    
#     text_data = json_data.get('text_data', [])
#     text_data_detailed = detailed_json.get('text_data', [])
    
#     # Process each item in text_data
#     for idx, (item, item_detailed) in enumerate(zip(text_data, text_data_detailed)):
#         text_associated = item.get('text_associated', '')
        
#         # Check if this text_associated is in our target list
#         if text_associated in target_text_associated:
#             # Get the existing label
#             label = item.get('label', '')
            
#             # Check if label is empty or consists of 1-2 digits or 1-2 letters
#             is_empty = not label or label.strip() == ''
#             is_digit = bool(re.match(r'^\d{1,2}$', label))
#             is_letter = bool(re.match(r'^[A-Za-z]{1,2}$', label))
            
#             # Check if label and text_associated are the same
#             is_same = label.strip().lower() == text_associated.strip().lower()
            
#             if is_empty or is_digit or is_letter or is_same:
#                 closest_tag = find_closest_tag(item, text_data)
#                 if closest_tag:
#                     assigned_text = closest_tag.get('label', '')
                    
#                     # Check if the closest tag's label is empty or just "tag" - if so, skip update
#                     if not assigned_text or assigned_text.strip().lower() == 'tag':
#                         print(f"Closest tag for {text_associated} (ID: {item['id']}) has no valid label. "
#                               f"Skipping update to preserve original value: {label}")
#                         continue
                    
#                     # Update label field with label from closest tag
#                     item['label'] = assigned_text
#                     item_detailed['label'] = assigned_text
#                     item_detailed['assigned_from_tag_id'] = closest_tag.get('id', '')
                    
#                     if is_same:
#                         print(f"Label and text_associated are the same '{label}' "
#                               f"(ID: {item['id']}). Label updated to: {assigned_text} "
#                               f"from closest tag (ID: {closest_tag.get('id', '')})")
#                     else:
#                         reason = 'empty' if is_empty else ('1-2 digits' if is_digit else '1-2 letters')
#                         print(f"Label '{label}' in {text_associated} (ID: {item['id']}) is "
#                               f"{reason}. Label updated to: {assigned_text} "
#                               f"from closest tag (ID: {closest_tag.get('id', '')})")
#                 else:
#                     # No tags found - keep original value if label and text match
#                     if is_same:
#                         print(f"Label and text_associated are the same '{label}' "
#                               f"(ID: {item['id']}), but no tags available. "
#                               f"Original value kept")
#                     else:
#                         reason = 'empty' if is_empty else ('1-2 digits' if is_digit else '1-2 letters')
#                         print(f"Label '{label}' in {text_associated} (ID: {item['id']}) is "
#                               f"{reason}, but no tags available")
#             else:
#                 # Label is valid and doesn't need reassignment
#                 print(f"Label in {text_associated} (ID: {item['id']}): {label} (kept as is)")
    
#     # Save updated original JSON file (overwrite input file)
#     with open(json_file_path, 'w', encoding='utf-8') as f:
#         json.dump(json_data, f, indent=4, ensure_ascii=False)
#     # with open(json_file_path, 'w') as f:
#     #     json.dump(json_data, f, indent=4)
    
#     print(f"\nInput JSON file updated: {json_file_path}")
      
#     #logger.info("closed tag assigned where necessary")
    
#     return detailed_json

########################################saikumar tag Identification code end #############################################
############################saikumar merge code###################################################
...
# -----------------------------
def build_components_json(detections, class_names, filename):
    comps = []
    for idx, (x1,y1,x2,y2,conf,cls) in enumerate(detections):
        comps.append({
            "id": idx,
...
# NOTE: you already provided extract_label_with_llm earlier. We'll adopt it but
# must ensure 'client' is set by the user/environment.
...
def extract_component_labels(components, cropped_image_path, filename):
    """
    Adds OCR text → comp['label']
    Moves YOLO class → comp['text_associated']
    Computes normalized bbox
    Creates Neo4j-compatible component JSON
    """
    labels_of_interest = {
        "Local Instrument","DCS","Line Number","Mechanical Interlock",
        "Instrument on Local Panel","Instrument in Back of Panel",
    ...
    for comp in components:
        ...
        # ----------------------------
        # 3️⃣ STORE YOLO CLASS NAME
        # ----------------------------
        comp["text_associated"] = comp["name"]

        # ----------------------------
        # 4️⃣ OCR GOES INTO 'label'
        # ----------------------------
        ...
        # Crop region
        roi = img[y1:y2, x1:x2]
        crop_path = os.path.join(crop_dir, f"comp_{comp['id']}.png")
        cv2.imwrite(crop_path, roi)

        # OCR extraction
        extracted_text = extract_label_with_llm(crop_path)

        if extracted_text:
            comp["label"] = extracted_text

    log("✔ Component OCR finished")
    return components
...
def push_to_neo4j(components, lines, connections, uri, user, pwd, filename):
    ...
    with driver.session() as session:
        ...
        SET 
            o.label = row.label,              // OCR value shown in circle
            o.caption = row.label,
...
def run_pipeline(
    ...
    # -------------------------------------
    # STEP 2 — Component detection (YOLO)
    # -------------------------------------
    
    raw_detections,full_img=detect_components(cropped,component_model)
    detections=remove_duplicate_bboxes(raw_detections)
    # detections, full_img = detect_components(
    #     cropped, component_model, comp_tile, comp_overlap, comp_conf
    # )

    # Debug component boxes
    debug_img = full_img.copy()
    for (x1, y1, x2, y2, conf, cls) in raw_detections:
    ...
    # Create overlay
    line_layer = full_img.copy()
    line_layer[mask == 255] = (0,255,255)

    blended = cv2.addWeighted(full_img, 0.7, line_layer, 0.3, 0)
    ...
    # -------------------------------------
    connections = build_simple_touch_connections_1(components, lines, margin=5)
    ...
    components = extract_component_labels(components, cropped_path, filename)
